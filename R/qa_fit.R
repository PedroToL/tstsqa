...
qa_fit <- function(donor_data, target_data, y_var, x_vars,
  ...
  #
  # donor_weights IS passed to this fit. An earlier version deliberately
  # left the fit unweighted, on the grounds that the first stage is a
  # prediction device whose coefficients are never interpreted. That is
  # true as far as it goes, but it breaks a property the downstream
  # diagnostics rely on: OLS forces Cov(eps, X) = 0 only in the metric the
  # model was fit in, so an unweighted fit paired with weighted population
  # moments leaves Cov_weighted(eps, X) nonzero. In the CASEN application
  # that wedge drove the lambda = rho*/rho identity off by up to 14% for
  # the circumstance with the smallest |rho|. Fitting weighted restores the
  # orthogonality in the same metric the moments are computed in.
  #
  # donor_weights defaults to rep(1, n) above when NULL, and passing
  # uniform weights to lm()/glm() is identical to not passing weights at
  # all, so the unweighted path is unchanged.
